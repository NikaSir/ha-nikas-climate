...
assert.equal(p.peerConnectionTone159(m),'nodata','unknown freshness must not produce a green peer lamp');
...
assert.equal(p.peerConnectionTone159(disconnected),'bad','unavailable climate must produce a red peer lamp');
...
p.connection=()=>({label:'Локально',fresh:'Данные актуальны',tone:'local'});
assert.equal(p.peerConnectionTone159(m),'ok','only confirmed current local data may produce a green peer lamp');
