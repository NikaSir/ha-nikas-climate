...
User override, UI 1.4.26 (2026-09-08): restore the previously working channel indication independently of freshness. Available Syncleo (including OFF) is green «Локально / Получено в HA»; explicit Syncleo `unavailable` is red «Нет связи / Нет данных»; `unknown`, missing entity or unverified platform is neutral «Нет данных / Нет данных». The peer lamps use the same availability signal. «Получено в HA» describes the HA state, not a confirmed device reply. Populated attributes may be restored or updated optimistically; neither their presence, a fresh HA timestamp nor successful Refresh proves device reception. Diagnostics reports freshness as «Не подтверждена» for the available channel. Fresh/stale device samples remain unverified. This explicit restoration supersedes the neutral surface rule introduced in UI 1.4.25, without changing any geometry, typography or device command.
