...
if (Panel && !Panel.prototype.__nikasUi132Patched) {
  ...
  Panel.prototype.statsChart = function (title, current, series, target=null, suffix="") {
  ...
  Panel.prototype.statistics = function (m) {
    ...
    return `<section class="card statistics-card">
      ...
      ${this.statsChart("Влажность · 24 ч", `Сейчас ${this.fmt(m.humidity,0)} %`, data?.humidity || [], null, "%")}
    </section>`;
  };
  ...
  Panel.prototype.render = function (...args) {
    ...
    if (root && !root.querySelector("style[data-nikas-ui132]")) {
      const style = document.createElement("style");
      style.dataset.nikasUi132 = "1";
      style.textContent = `
  ...
  Panel.prototype.patch = function (...args) {
    ...
    if (version) version.textContent = `UI v${PATCH_UI_VERSION}`;
    if (this._tab === "statistics") {
      const room = [{key:"living",title:"Зал"},{key:"veranda",title:"Веранда"}].find((r)=>r.key===this._selected);
      if (room) {
        const m = this.roomModel(room);
        if (!this._history[m.room.key] && !this._historyLoading[m.room.key]) this.ensureHistory(m);
      }
    }
    return result;
  };

  Panel.prototype.__nikasUi132Patched = true;
}
